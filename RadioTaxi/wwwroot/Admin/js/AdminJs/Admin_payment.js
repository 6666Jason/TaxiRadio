var payment_app = new Vue({
    ...
    mounted() {
        ...
        axios.get("/Admin/PaymentUser/GetAllListDataRecharage")
            .then((response) => {
                ...
                $("#payment_table").DataTable({
                    ...
                    language: {
                        ...
                        "editor": {
                            ...
                            "error": {
                                "system": "Đã xảy ra Error hệ thống (&lt;a target=\"\\\" rel=\"nofollow\" href=\"\\\"&gt;Thêm thông tin&lt;\/a&gt;)."
                            },
                            "multi": {
    ...
    methods: {
        ...
        loadItems() {
            if (this.loadCheck == false) {
                ...
                axios.get("/Admin/PaymentUser/GetCourse")
                    .then((response) => {
                        this.dataCourseList = response.data;
                        return Promise.resolve();
                    })
                axios.get("/Admin/PaymentUser/Customer")
                    .then((response) => {
                        this.dataUserList = response.data;
                        return Promise.resolve();
                    }).finally(() => {
                        $('#preloader').fadeOut();
                    });
        ...
        async addCourse() {
            try {
                ...
                formData.append('PackID', this.CourseID);
                formData.append('UserName', this.nameUser);

                await axios.post('/Admin/PaymentUser/CreatePayment', formData,
                    {
                        headers: {
        ...
        async handleTransaction(userNap) {
            ...
            try {
                if (userNap.status) {
                    ...
                    userNap.status = false;
                    formData.append('id', userNap.id);

                    axios.post('/Admin/PaymentUser/UpdateRecharage', formData, {
                        headers: {
                        ...
                        if (res.data) {
                            ...
                            Swal.fire({
                                icon: 'error',
                                title: 'Error',
                                text: 'Error trong quá trình gửi',
                                confirmButtonText: 'OK'
                            }).then((result) => {
                                if (result.isConfirmed) {
                                    window.location.reload();
                    ...
                    axios.post('/Admin/PaymentUser/UpdateRecharageSuccess', formData, {
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded'
                        }
            ...
            if (this.id != null || this.id != 0) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        axios.post('/Admin/PaymentUser/DeletePack', formData, {
                            headers: {
                                'Content-Type': 'application/x-www-form-urlencoded'
                            }
                        }).then(response => {
